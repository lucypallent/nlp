# currently not running with the grid search
...
train_df = train_df.merge(train_tfidf, how='left', on=['articleBody', 'headline'])
...
print('saving model')
model.save_model(os.path.join(SAVE_DIR, 'all-XGB.json'))
# model2.save_model(os.path.join(SAVE_DIR, 'all-XGB2.json'))
print('model saved')
